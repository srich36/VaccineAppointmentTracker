...
sort_order = SortOrder.APPOINTMENTS_LAST_MODIFIED
desired_types = set([VaccineTypes.UNKNOWN, VaccineTypes.PFIZER, VaccineTypes.MODERNA, VaccineTypes.ALL, VaccineTypes.JJ])
#### CONFIGURATION ####
...
close_sites = [ site for site in availble_vaccines if under_distance_threshold(site) ]
for close_site in close_sites:
    close_site['distance_away'] = get_distance_away(close_site)
    appointments_last_modified = parser.parse(close_site["properties"]["appointments_last_modified"])
    appointments_last_fetched = parser.parse(close_site["properties"]["appointments_last_fetched"])
...
print(f'{len(close_sites)} sites found')
print('-' * 80)
